...
void LIVMapper::readParameters()
{
  ...
  this->declare_parameter<double>("outlier_threshold2", 100);
  this->get_parameter("outlier_threshold2", outlier_threshold2);
  this->declare_parameter<double>("outlier_threshold3", 100);
  this->get_parameter("outlier_threshold3", outlier_threshold3);
  ...
  this->declare_parameter<vector<double>>("extrin_calib.extrinsic_T", vector<double>());
  this->get_parameter("extrin_calib.extrinsic_T", extrinT);
  this->declare_parameter<vector<double>>("extrin_calib.extrinsic_R", vector<double>());
  this->get_parameter("extrin_calib.extrinsic_R", extrinR);
  this->declare_parameter<vector<double>>("extrin_calib.Pcl", vector<double>());
  this->get_parameter("extrin_calib.Pcl", cameraextrinT);
  this->declare_parameter<vector<double>>("extrin_calib.Rcl", vector<double>());
...
// sheng launch 读取参数
  ...
  this->declare_parameter<double>("scale_factor", 3.4);
  this->get_parameter("scale_factor", scale_factor);
  this->declare_parameter<double>("scale_factor2", 3.4);
  this->get_parameter("scale_factor2", scale_factor2);
  this->declare_parameter<double>("root_voxel_size", 0.5);
  this->get_parameter("root_voxel_size", root_voxel_size);
  this->declare_parameter<int>("octree_max_level", 3);
  this->get_parameter("octree_max_level", octree_max_level);

  this->declare_parameter<int>("gs.gs_iterations", 4);
  this->get_parameter("gs.gs_iterations", gs_iterations);
  this->declare_parameter<int>("gs.border_gs", 4);
  this->get_parameter("gs.border_gs", border_gs);
  this->declare_parameter<int>("gs.plot_gs_render", 0);
  this->get_parameter("gs.plot_gs_render", plot_gs_render);
  this->declare_parameter<double>("gs.gs_position_lr", 0.00016);
  this->get_parameter("gs.gs_position_lr", gs_position_lr);
  this->declare_parameter<double>("gs.gs_feature_lr", 0.005);
  this->get_parameter("gs.gs_feature_lr", gs_feature_lr);
  this->declare_parameter<double>("gs.gs_opacity_lr", 0.05);
  this->get_parameter("gs.gs_opacity_lr", gs_opacity_lr);
  this->declare_parameter<double>("gs.gs_scaling_lr", 0.001);
  this->get_parameter("gs.gs_scaling_lr", gs_scaling_lr);
  this->declare_parameter<double>("gs.gs_rotation_lr", 0.001);
  this->get_parameter("gs.gs_rotation_lr", gs_rotation_lr);
...
void LIVMapper::initializeSubscribersAndPublishers(image_transport::ImageTransport &it)
{
  if (p_pre->lidar_type == AVIA)
  {
    sub_pcl_livox = this->create_subscription<livox_ros_driver2::msg::CustomMsg>(
        lid_topic, rclcpp::QoS(200000), std::bind(&LIVMapper::livox_pcl_cbk, this, std::placeholders::_1));
  }
  else
  {
    sub_pcl = this->create_subscription<sensor_msgs::msg::PointCloud2>(
        lid_topic, rclcpp::QoS(200000), std::bind(&LIVMapper::standard_pcl_cbk, this, std::placeholders::_1));
  }
  sub_imu = this->create_subscription<sensor_msgs::msg::Imu>(
      imu_topic, rclcpp::QoS(200000), std::bind(&LIVMapper::imu_cbk, this, std::placeholders::_1));
  sub_img = this->create_subscription<sensor_msgs::msg::Image>(
      img_topic, rclcpp::QoS(200000), std::bind(&LIVMapper::img_cbk, this, std::placeholders::_1));
  ...
  imu_prop_timer = this->create_wall_timer(std::chrono::duration<double>(0.004), std::bind(&LIVMapper::imu_prop_callback, this));
...
{    
  ...
  euler_cur = RotMtoEuler(_state.rot_end);
  {
    ...
    tf2_quat.setRPY(euler_cur(0), euler_cur(1), euler_cur(2));
    geoQuat = tf2::toMsg(tf2_quat);
  }
...
void LIVMapper::publish_odometry(const rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr &pubOdomAftMapped)
{
  ...
  odomAftMapped.header.stamp = this->now(); //.fromSec(last_timestamp_lidar);
